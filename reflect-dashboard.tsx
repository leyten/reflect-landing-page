  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/30 to-yellow-50/40 relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none">
        {/* Large Floating Orbs */}
        <div className="absolute top-10 left-10 w-64 h-64 bg-gradient-to-br from-yellow-300/30 to-yellow-500/15 rounded-full blur-3xl animate-pulse duration-[8000ms]"></div>
        <div className="absolute top-20 right-10 w-48 h-48 bg-gradient-to-br from-blue-300/25 to-blue-500/10 rounded-full blur-2xl animate-pulse duration-[6000ms] delay-1000"></div>
        <div className="absolute bottom-20 left-1/4 w-56 h-56 bg-gradient-to-br from-green-300/20 to-green-500/10 rounded-full blur-3xl animate-pulse duration-[10000ms] delay-2000"></div>
        <div className="absolute bottom-10 right-1/3 w-72 h-72 bg-gradient-to-br from-purple-300/15 to-purple-500/8 rounded-full blur-3xl animate-pulse duration-[12000ms] delay-3000"></div>

        {/* Moving Gradient Waves */}
        <div className="absolute inset-0">
          <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-r from-yellow-200/10 via-transparent to-blue-200/10 animate-pulse duration-[15000ms]"></div>
          <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-l from-green-200/8 via-transparent to-purple-200/8 animate-pulse duration-[18000ms] delay-2000"></div>
        </div>
        ...
        {Array.from({ length: 8 }).map((_, i) => (
          <div
            key={i}
            className={`absolute w-1.5 h-1.5 bg-yellow-400/40 rounded-full animate-bounce shadow-sm`}
            style={{
              left: `${15 + i * 10}%`,
              top: `${20 + (i % 3) * 25}%`,
        ...
        {Array.from({ length: 6 }).map((_, i) => (
          <div
            key={`line-${i}`}
            className="absolute bg-gradient-to-r from-transparent via-yellow-400/15 to-transparent h-px animate-pulse"
            style={{
              width: `${150 + i * 40}px`,
              left: `${i * 18}%`,
              top: `${25 + i * 12}%`,
              animationDelay: `${i * 2000}ms`,
              animationDuration: `${5000 + i * 800}ms`,
        ...
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={`circle-${i}`}
            className="absolute border border-yellow-400/10 rounded-full animate-ping"
            style={{
              width: `${80 + i * 25}px`,
              height: `${80 + i * 25}px`,
              left: `${25 + i * 20}%`,
              ...
              animationDuration: `${4000 + i * 1500}ms`,
            }}
          ></div>
        ))}

        {/* Subtle Shimmer Effect */}
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent -skew-x-12 animate-pulse duration-[8000ms] delay-1000"></div>
      </div>
      ...
      {/* Header */}
      <header className="bg-white/80 backdrop-blur-sm border-b border-gray-200/50 px-6 py-5 sticky top-0 z-50 shadow-sm">
