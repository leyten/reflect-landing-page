...
export default function WalletInfo() {
  ...
  const refreshBalances = async () => {
    ...
    for (const wallet of wallets) {
      // Check if it's a Solana wallet by looking at the address format
      if (wallet.address && wallet.address.length === 44) {
        try {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-zinc-100 p-6">
      ...
      <div className="space-y-4">
        {wallets.map((wallet) => {
          // Determine if it's a Solana wallet by address length
          const isSolanaWallet = wallet.address && wallet.address.length === 44
